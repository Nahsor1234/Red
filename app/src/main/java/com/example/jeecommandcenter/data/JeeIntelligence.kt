...
class JeeIntelligence(
    ...
    fun revisionRecommendations(limit: Int = 12): List<RevisionRecommendation> {
        ...
        return JeeCatalog.chapters.mapNotNull { chapter ->
            ...
            val days = state.lastStudiedAt.let { studied ->
                val last = if (studied > 0L) jee.getRevisionHistory(chapter.id)
                    .firstOrNull()?.reviewedAt ?: 0L else 0L
                if (last <= 0L) 999L
                else ChronoUnit.DAYS.between(
                    Instant.ofEpochMilli(last).atZone(zone).toLocalDate(), today
                )
            }
            val attempts = learning.getQuestionAttempts().filter { it.chapterId == chapter.id }
            val accuracy = if (attempts.size >= 3)
                attempts.count { it.correct }.toFloat() / attempts.size else null
            val mistakes = learning.getMistakes().count {
            ...
            if (due != null) {
                val overdue = ChronoUnit.DAYS.between(
                    LocalDate.parse(due.revision.dueDate), today
                ).coerceAtLeast(0)
                score += 25 + minOf(overdue, 14).toInt() * 3
                reasons += if (overdue > 0) "overdue by ${overdue}d" else "revision is due"
            } else if (days >= 7) {
                score += 20
                reasons += "last revision was ${days}d ago"
            }
            if (state.confidence <= 2) {
                score += (3 - state.confidence) * 12
                reasons += "confidence ${state.confidence}/5"
            }
            if (accuracy != null && accuracy < 0.65f) {
                score += ((0.65f - accuracy) * 70f).toInt()
                reasons += "${(accuracy * 100).toInt()}% recent accuracy"
            }
            if (mistakes >= 2) {
                score += mistakes * 6
                reasons += "${mistakes} unresolved mistakes"
            }
            ...
            RevisionRecommendation(
                chapter = chapter,
                level = when {
                    score >= 55 -> PriorityLevel.HIGH
                    score >= 30 -> PriorityLevel.MEDIUM
    ...
    fun weakChapters(limit: Int = 8): List<WeakChapter> {
        ...
        return JeeCatalog.chapters.mapNotNull { chapter ->
            ...
            val accuracy = if (chapterAttempts.size >= 3)
                chapterAttempts.count { it.correct }.toFloat() / chapterAttempts.size else null
            val unresolved = mistakes.count { it.chapterId == chapter.id && !it.resolved }
            val gapDays = if (state.lastStudiedAt <= 0L) 999L else
                ChronoUnit.DAYS.between(
                    Instant.ofEpochMilli(state.lastStudiedAt).atZone(zone).toLocalDate(),
                    LocalDate.now()
                )
            ...
            val signals = buildList {
                ...
                if (accuracy != null && accuracy < 0.65f) {
                    add("${(accuracy * 100).toInt()}% recent accuracy")
                }
                if (gapDays >= 10) add("${gapDays} days since study")
            }
            if (signals.size < 2) null else
                WeakChapter(chapter, state.confidence, accuracy, unresolved, signals.take(4))
        }.sortedWith(
            compareByDescending<WeakChapter> { it.signals.size }.thenBy { it.confidence }
        ).take(limit)
    }

    fun dailyPriorities(limit: Int = 5): List<DailyPriority> {
        val priorities = mutableListOf<DailyPriority>()

        revisionRecommendations(3).forEach { item ->
            priorities += DailyPriority(
                "Revise ${item.chapter.name}",
        ...
        weakChapters(3).forEach { weak ->
            if (priorities.any { it.chapterId == weak.chapter.id }) return@forEach
            priorities += DailyPriority(
                "Strengthen ${weak.chapter.name}",
                weak.chapter.subject,
                weak.chapter.id,
                45,
                if (weak.signals.size >= 3) PriorityLevel.HIGH else PriorityLevel.MEDIUM,
                weak.signals.take(2).joinToString(" + ")
            )
        }
        ...
        jee.getTasks().filter { !it.done && it.dueDay == "Today" }.take(3).forEach { task ->
            priorities += DailyPriority(
                ...
                "unfinished task due today"
            )
        }

        return priorities.sortedWith(
            compareByDescending<DailyPriority> {
                when (it.level) {
        ...
        ).distinctBy { it.chapterId ?: it.title }.take(limit)
    }
    ...
    fun weeklyReview(): WeeklyReview {
        ...
        val accuracy = if (attempts.isEmpty()) 0f
            else attempts.count { it.correct }.toFloat() / attempts.size
        ...
        return WeeklyReview(
            studyMinutes = sessions.sumOf { it.minutes },
            sessions = sessions.size,
            revisions = revisions,
            tests = tests,
            mistakes = mistakes,
            averageAccuracy = accuracy,
            weakChapters = weakChapters(5).map { it.chapter.name },
            improvementFocus = focus.take(4)
        )
    }
}
