...
class AiEngine(private val settings: AiSettingsRepository) {
    ...
    suspend fun stream(prompt: String, systemInstruction: String? = null, onChunk: suspend (String) -> Unit): AiResult = withContext(Dispatchers.IO) {
        val key = settings.apiKey() ?: return@withContext AiResult(false, error = "Add an API key in Settings → AI Hub.")
        val config = settings.getConfig()
        runCatching {
    ...
    suspend fun generateQuiz(subject: String, chapter: String?, difficulty: String, count: Int): List<Question> {
        val prompt = "Create exactly $count JEE practice MCQs for $subject${chapter?.let { " chapter '$it'" } ?: ""}. Difficulty: $difficulty. Return ONLY valid JSON array. Each object: {\"prompt\":string,\"options\":[4 strings],\"correctIndex\":0-3,\"explanation\":string,\"topic\":string}. No markdown."
        val result = ask(prompt, "You generate accurate educational multiple-choice questions. Exactly one option must be correct. Never invent exam claims.")
        if (!result.success) return emptyList()
        return parseQuiz(result.text, subject, chapter ?: "General", difficulty)
    }

    private fun parseQuiz(raw: String, subject: String, chapter: String, difficulty: String): List<Question> {
        val clean = raw.substringAfter("[").let { "[" + it.substringBeforeLast("]") + "]" }
        return runCatching {
            ...
            List(array.length()) { i ->
                ...
                val d = when (difficulty.lowercase()) { "easy" -> 1; "hard" -> 5; else -> 3 }
                Question("ai_${System.currentTimeMillis()}_$i", subject, "ai_${subject.lowercase()}", chapter, o.optString("topic", "Core concepts"), o.getString("prompt"), List(4) { options.getString(it) }, o.getInt("correctIndex").coerceIn(0, 3), o.optString("explanation", "Review the underlying concept."), d, "AI generated")
            }
        }.getOrDefault(emptyList())
    }

    private fun callGemini(key: String, model: String, prompt: String, systemInstruction: String?): AiResult {
        ...
        return postJson(URL("https://generativelanguage.googleapis.com/v1beta/models/$model:generateContent"), mapOf("x-goog-api-key" to key), body) { response ->
            response.optJSONArray("candidates")?.optJSONObject(0)?.optJSONObject("content")?.optJSONArray("parts")?.optJSONObject(0)?.optString("text")?.takeIf { it.isNotBlank() } ?: throw IllegalStateException("Gemini returned no text.")
        }
    }
    ...
    private fun callOpenAiCompatible(key: String, endpoint: String, model: String, prompt: String, systemInstruction: String?): AiResult {
        ...
        return postJson(URL(endpoint), mapOf("Authorization" to "Bearer $key"), JSONObject().put("model", model.trim()).put("messages", messages)) { response ->
            response.optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("message")?.optString("content")?.takeIf { it.isNotBlank() } ?: throw IllegalStateException("Provider returned no text.")
        }
    }

    private fun streamOpenAiCompatible(key: String, endpoint: String, model: String, prompt: String, systemInstruction: String?, onChunk: suspend (String) -> Unit): String {
        require(endpoint.startsWith("https://")) { "API endpoint must start with https://" }
        val messages = JSONArray()
        if (!systemInstruction.isNullOrBlank()) messages.put(JSONObject().put("role", "system").put("content", systemInstruction))
        messages.put(JSONObject().put("role", "user").put("content", prompt))
        val connection = open(URL(endpoint), mapOf("Authorization" to "Bearer $key"))
        connection.outputStream.use { it.write(JSONObject().put("model", model).put("messages", messages).put("stream", true).toString().toByteArray()) }
        check(connection.responseCode in 200..299) { "AI provider HTTP ${connection.responseCode}" }
        ...
        connection.inputStream.bufferedReader().useLines { lines ->
            lines.forEach { line ->
                ...
                val delta = runCatching { JSONObject(data).optJSONArray("choices")?.optJSONObject(0)?.optJSONObject("delta")?.optString("content") ?: "" }.getOrDefault("")
                if (delta.isNotEmpty()) { out.append(delta); kotlinx.coroutines.runBlocking { onChunk(delta) } }
            }
        }
    ...
    private fun streamGemini(key: String, model: String, prompt: String, systemInstruction: String?, onChunk: suspend (String) -> Unit): String {
        val body = JSONObject().apply {
            if (!systemInstruction.isNullOrBlank()) put("systemInstruction", JSONObject().put("parts", JSONArray().put(JSONObject().put("text", systemInstruction))))
            put("contents", JSONArray().put(JSONObject().put("parts", JSONArray().put(JSONObject().put("text", prompt)))))
        }
        val url = URL("https://generativelanguage.googleapis.com/v1beta/models/$model:streamGenerateContent?alt=sse")
        val connection = open(url, mapOf("x-goog-api-key" to key))
        connection.outputStream.use { it.write(body.toString().toByteArray()) }
        check(connection.responseCode in 200..299) { "Gemini HTTP ${connection.responseCode}" }
        val out = StringBuilder()
        connection.inputStream.bufferedReader().useLines { lines ->
            lines.forEach { line ->
                if (!line.startsWith("data:")) return@forEach
                val data = line.removePrefix("data:").trim()
                val delta = runCatching { JSONObject(data).optJSONArray("candidates")?.optJSONObject(0)?.optJSONObject("content")?.optJSONArray("parts")?.optJSONObject(0)?.optString("text") ?: "" }.getOrDefault("")
                if (delta.isNotEmpty()) { out.append(delta); kotlinx.coroutines.runBlocking { onChunk(delta) } }
            }
        }
        connection.disconnect()
    ...
    private fun open(url: URL, headers: Map<String, String>): HttpURLConnection = (url.openConnection() as HttpURLConnection).apply {
        requestMethod = "POST"; connectTimeout = 15_000; readTimeout = 120_000; doOutput = true; setRequestProperty("Content-Type", "application/json"); headers.forEach { (n, v) -> setRequestProperty(n, v) }
    }
    ...
    private fun postJson(url: URL, headers: Map<String, String>, body: JSONObject, extractor: (JSONObject) -> String): AiResult {
        ...
        if (status !in 200..299) throw IllegalStateException(response.optString("message").ifBlank { response.optString("error").ifBlank { "HTTP $status" } })
